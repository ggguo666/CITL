...
from imports.cnn_Dataset import ASD_ADHD_Dataset_official
from torch import nn
from net.cnn import CNN
# from net.cnn2 import CNN
from imports.utils import train_val_test_split_ABIDE_ADHD
...
import numpy as np
# 定义保存模型的文件夹路径
checkpoint_dir = "/home/user/data/gsj/111111111/251/guo/ABIDE_ADHDBaseModel/"
# 创建一个文件夹用于保存模型
...
def main():
    ...
    path = "/home/user/data/gsj/ABIDEI_ADHD"
    path1 = "/home/user/data/gsj/ABIDE_116/abide_116/dfc"
    path2 = "/home/user/data/gsj/ADHD/116/adhd200_116/dfc"
    ...
    for fold in range(0, fold):
        # 加载数据集示例
        dataset = ASD_ADHD_Dataset_official(path1, path2, save_dir=path)
        print(dataset)

        tr_index, val_index, te_index = train_val_test_split_ABIDE_ADHD(fold=fold)
        ...
        def test(loader):
            ...
            return epoch_spe, epoch_sen, epoch_acc, loss_all / len(test_dataset), auc
        #######################################################################################
        ############################   Model Training #########################################
        #######################################################################################
    ...
    #######################################################################################
        ...
        # 定义保存模型的文件夹路径
        checkpoint_dir = "/home/user/data/gsj/111111111/251/guo/ABIDE_ADHDBaseModel/"
        best_model_dir = "/home/user/data/gsj/111111111/251/guo/ABIDE_ADHDBset_BaseModel/"
        best_model_path = ""
        test_accuracy1 = 0
        test_ave = 0
        ...
        for checkpoint_file in checkpoint_files:
            ...
            model.eval()
            with torch.no_grad():
                sen, spe, test_accuracy, test_l,auc = test(test_loader)
        ...
        for checkpoint_file1 in checkpoint_files:
            checkpoint_path1 = os.path.join(checkpoint_dir, checkpoint_file1)
    ...
    print(spe_list)
    print(auc_list)
    print("Test aveAcc: {:.7f}".format(test_accuracyave / 10))
