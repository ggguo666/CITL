...
from imports.cnn_Dataset import abide_ave_Dataset_official
from torch import nn
from net.cnn import CNN
from net.AE2abide_stage3 import Autoencoder
from imports.utils import train_val_test_split_stage3_ABIDE, train_test_split_stage3
...
def main():
    ...
    parser.add_argument('--dataroot', type=str, default='/home/user/data/gsj/ABIDE_116/abide_116/dfc/ave',
                        help='root directory of the dataset')
    parser.add_argument('--fold', type=int, default=10, help='training which fold')

    parser.add_argument('--lr1', type=float, default=0.00008, help='learning rate')  # 0.000005
    parser.add_argument('--weightdecay1', type=float, default=0.0005, help='regularization')  # 0.005

    parser.add_argument('--lr', type=float, default=0.00004, help='learning rate')  # v0.00005
    parser.add_argument('--weightdecay', type=float, default=0.00005, help='regularization')  # 0.0005

    parser.add_argument('--stepsize', type=int, default=100, help='scheduler step size')
    parser.add_argument('--gamma', type=float, default=1, help='scheduler shrinking rate')
    ...
    for fold in range(0, fold):
        # 加载数据集示例
        dataset = abide_ave_Dataset_official(path, save_dir=path)
        # print(dataset)
        # tr_index, val_index, te_index = train_val_test_split_stage3_ABIDE(fold=fold, seed=seed2)
        tr_index, te_index = train_test_split_stage3(fold=fold, seed=seed2)
